#!/usr/bin/env bash
# Verify test keys against the expected count saved by insert-data.sh.
...
if [ ! -f "$KEYCOUNT_FILE" ]; then
    echo "ERROR: .keycount not found. Run insert-data.sh first."
    exit 1
fi
...
echo "Verifying data against master (localhost:$MASTER_PORT)  [expected $EXPECTED keys]"
echo ""
...
for i in $(seq 1 10 "$EXPECTED"); do
    ...
    actual_val=$(redis-cli -p "$MASTER_PORT" GET "$key" 2>/dev/null | tr -d '\r')
